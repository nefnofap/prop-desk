...
// Evaluate a SPECIFIC line (the one the sportsbook actually posted).
// Returns both sides so you can see which way the book's line leans.
...
// Auto-suggest best bets — tests BOTH over and under for each stat,
// and returns whichever side has the higher hit probability.
export function autoSuggest(logs, window, odds) {
  ...
  return Object.entries(STAT_TYPES).map(([key, def]) => {
    ...
    if (!vals.length) return null;
    const avg = vals.reduce((a, b) => a + b, 0) / vals.length;
    const line = Math.max(0.5, Math.round((avg - 0.25) * 2) / 2);
    const over = weightedShrunkProb(vals, line, "over");
    const under = weightedShrunkProb(vals, line, "under");
