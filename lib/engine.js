...
// Minimum line a sportsbook will realistically post for each stat.
// Below this, books don't offer the prop (e.g. nobody bets "Under 0.5 blocks").
// A book typically only lists a stat when the line clears these floors.
const BOOK_MIN_LINE = {
  pts: 7.5, reb: 3.5, ast: 2.5, pra: 14.5, pr: 9.5, pa: 9.5, ra: 5.5,
  stl: 1.5, blk: 1.5, fg3m: 1.5, to: 1.5,
};
...
// Is this player even a prop candidate? Books only post props on rotation
// players with real minutes. We approximate "real role" from scoring volume.
export function isPropCandidate(logs, window) {
  ...
  // Starter/key rotation bar: ~20+ min or 8+ ppg. Bench scrubs fall out.
  return avgMinNum >= 18 || avgPts >= 8;
}

// Auto-suggest — now only returns BETTABLE props:
// 1. Player must be a prop candidate (real minutes)
// 2. Line must clear the book's minimum for that stat
// 3. The app's line is bumped to at least the book floor, near the player's median
export function autoSuggest(logs, window, odds, opts = {}) {
  ...
  return Object.entries(STAT_TYPES).map(([key, def]) => {
    ...
    // A book only posts this stat if the player's average is meaningfully
    // above the floor. Skip stats where they're too low-volume to be listed.
