...
// ════════ TEAM BETS ════════
// All driven by the line YOU type from 747. Uses recent team game results.
// games: [{ margin (+ = won by), total, won, myScore, oppScore }]

function shrink(hits, n, k = 6) {
  if (!n) return 0.5;
  return (hits + k * 0.5) / (n + k);
}

// Money line: how often this team won outright.
export function evalMoneyline(games, odds) {
  const n = games.length;
  const wins = games.filter(g => g.won).length;
  const p = shrink(wins, n);
  return { type: "moneyline", wins, n, p, edge: p - impliedProb(odds) };
}

// Spread: type the team's line, e.g. -6.5 (favored) or +4.5 (underdog).
// Cover = (margin + line) > 0.
export function evalSpread(games, line, odds) {
  const n = games.length;
  const covers = games.filter(g => g.margin + line > 0).length;
  const push = games.filter(g => g.margin + line === 0).length;
  const p = shrink(covers, n);
  return { type: "spread", line, covers, push, n, p, edge: p - impliedProb(odds),
    avgMargin: n ? games.reduce((a, g) => a + g.margin, 0) / n : 0 };
}

// Total points: combined score over/under.
export function evalTotal(games, line, side, odds) {
  const n = games.length;
  const overs = games.filter(g => g.total > line).length;
  const unders = games.filter(g => g.total < line).length;
  const hits = side === "over" ? overs : unders;
  const p = shrink(hits, n);
  return { type: "total", line, side, overs, unders, n, p, edge: p - impliedProb(odds),
    avgTotal: n ? games.reduce((a, g) => a + g.total, 0) / n : 0 };
}

// This team's own points over/under.
export function evalTeamTotal(games, line, side, odds) {
  const n = games.length;
  const overs = games.filter(g => g.myScore > line).length;
  const unders = games.filter(g => g.myScore < line).length;
  const hits = side === "over" ? overs : unders;
  const p = shrink(hits, n);
  return { type: "teamtotal", line, side, overs, unders, n, p, edge: p - impliedProb(odds),
    avgScore: n ? games.reduce((a, g) => a + g.myScore, 0) / n : 0 };
}

// Odd/even on this team's points.
export function evalOddEven(games, which, odds) {
  const n = games.length;
  const hits = games.filter(g => (g.myScore % 2 === 0) === (which === "even")).length;
  const p = shrink(hits, n);
  return { type: "oddeven", which, hits, n, p, edge: p - impliedProb(odds) };
}
