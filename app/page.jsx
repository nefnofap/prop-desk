...
export default function PropDesk() {
  ...
  const suggestions = useMemo(() => draft ? autoSuggest(draft.logs, draft.window, draft.odds).slice(0, 5) : [], [draft]);
    ...
    <div style={{ minHeight: "100vh", background: "var(--bg)", color: "var(--tx)", fontFamily: "Inter,sans-serif", paddingBottom: 120 }}>
      ...
      <header style={{ borderBottom: "1px solid var(--line)", padding: "14px 16px 12px" }}>
        ...
        <div style={{ display: "flex", gap: 10, marginTop: 10, flexWrap: "wrap", alignItems: "center" }}>
          ...
          <label style={lbl}>Default odds (decimal)
      ...
      <div style={{ maxWidth: 640, margin: "0 auto", padding: "0 14px" }}>
        ...
        {draft && draftProb && (
          <section style={{ ...panel, border: "1px solid rgba(251,191,36,.4)" }}>
              ...
              <div style={{ display: "flex", gap: 6, flexWrap: "wrap" }}>
                {suggestions.map(s => (
                  <button key={s.key} onClick={() => setDraft({ ...draft, stat: s.key, line: s.line })}
                    ...
                    {s.short} o{s.line} · {fmtPct(s.p)} ({s.hits}/{s.n}){s.edge > 0.03 ? " 🔥" : s.edge > 0 ? " ✓" : ""}
                  </button>
                ))}
              </div>
            </div>
            ...
            <div style={{ fontFamily: "'IBM Plex Mono',monospace", fontSize: 12, color: "var(--mut)", display: "flex", gap: 14, flexWrap: "wrap" }}>
              ...
              <span style={{ color: draftEdge >= 0 ? "var(--green)" : "var(--red)" }}>edge: {draftEdge >= 0 ? "+" : ""}{fmtPct(draftEdge)}</span>
              ...
              <div key={l.id} style={{ display: "flex", justifyContent: "space-between", alignItems: "center", paddingBottom: 8, borderBottom: "1px solid var(--line)" }}>
                <div>
                  ...
                  <div style={{ fontFamily: "'IBM Plex Mono',monospace", fontSize: 11, color: "var(--mut)" }}>
                    {l.side} {l.line} {STAT_TYPES[l.stat].short} · {l.odds} ({decimalToAmerican(l.odds)}) · <span style={{ color: l.edge >= 0 ? "var(--green)" : "var(--red)" }}>{l.edge >= 0 ? "+" : ""}{fmtPct(l.edge)}</span> · P {fmtPct(l.p)}
                  </div>
                </div>
              ...
              return (
                <div key={pl.name} style={{ ...card, borderColor: ev >= 0 ? "rgba(52,211,153,.3)" : "var(--line)" }}>
                  <div style={{ display: "flex", justifyContent: "space-between", alignItems: "baseline" }}>
                    ...
                    <span style={{ fontFamily: "'IBM Plex Mono',monospace", fontSize: 12, color: ev >= 0 ? "var(--green)" : "var(--red)" }}>EV {ev >= 0 ? "+" : ""}{ev.toFixed(3)}</span>
                  </div>
                  {pl.legs.map((l, i) => (
                  ...
                  <div style={{ display: "grid", gridTemplateColumns: "repeat(3,1fr)", gap: 8 }}>
                    {[["Combined", `${dec.toFixed(2)}x`], ["Hit P*", fmtPct(adjP)], ["Payout", fmtCash(stake * dec, sym)]].map(([k, v]) => (
                  ...
                  <div style={{ fontSize: 11, color: "var(--mut)" }}>
                    100 plays at {fmtCash(stake, sym)}: ~{Math.round(adjP * 100)} wins → {fmtCash((adjP * (dec - 1) - (1 - adjP)) * 100 * stake, sym)} expected.
                    {extraSameTeam > 0 && " Same-team haircut applied."}
                  </div>
                </div>
              );
            })}
        ...
        {legs.length >= 2 && parlays.length === 0 && (
          <div style={{ ...panel, color: "var(--mut)", fontSize: 13 }}>
            No parlay recommended — fewer than 2 legs show positive edge. That's the system working.
          </div>
        )}
            ...
            <div style={{ padding: "0 16px 16px", display: "flex", flexDirection: "column", gap: 10 }}>
              {legs.map(l => (
                <div key={l.id} style={{ display: "flex", justifyContent: "space-between", fontFamily: "'IBM Plex Mono',monospace", fontSize: 12 }}>
                  <span>{l.player.name} {l.side} {l.line} {STAT_TYPES[l.stat].short}</span>
              ...
              <div style={{ fontFamily: "'IBM Plex Mono',monospace", fontSize: 11, color: "var(--mut)" }}>
                {slip.dec.toFixed(3)}x · hit P {fmtPct(slip.adjP)} · EV/{sym}1 <span style={{ color: slip.ev >= 0 ? "var(--green)" : "var(--red)" }}>{slip.ev >= 0 ? "+" : ""}{slip.ev.toFixed(3)}</span>
              </div>
            </div>
          )}
        </div>
